...
class AccountMove(models.Model):
    ...
    def _is_eligible_for_early_payment_discount(self, currency, reference_date):
        ...
        return self.currency_id == currency \
            ...
            and (not reference_date or reference_date <= self.invoice_payment_term_id._get_last_discount_date(
                self.invoice_date)) \
    ...
    def copy(self, default=None):
        ...
        message_content = _('This entry has been reversed from %s', self._get_html_link()) if default.get(
            'reversed_entry_id') else _('This entry has been duplicated from %s', self._get_html_link())
    ...
    def write(self, vals):
        ...
        for move in self:
            ...
            0, 1) and 'journal_id' in vals and move.journal_id.id != vals['journal_id'] and not move.quick_edit_mode):
                raise UserError(
        ...
        for file_data in file_data_list:
            ...
            if decoder:
                try:
                    ...
                    message = _("Error importing attachment '%s' as invoice (decoder=%s)", file_data['filename'],
                                decoder.__name__)
        ...
        if self.move_type != 'entry':
            ...
            recipient_group = (
                ...
                lambda pdata: pdata['id'] in local_msg_vals.get('partner_ids', []) and pdata[
                    'id'] != self.partner_id.id and pdata['type'] != 'user',
                {
        ...
        for _company, moves in all_moves.grouped('company_id').items():
            ...
            for partner_id, partner_moves in moves_by_partner.items():
                ...
                partner_moves_error = partner_moves.filtered(
                    lambda m: m.send_and_print_values and m.send_and_print_values.get('error'))
    ...
    def _get_invoice_next_payment_values(self):
        ...
        if epd_installment and discount_date:
            ...
            days_left = (discount_date - fields.Date.context_today(
    ...
    def _generate_pdf_and_send_invoice(self, template, force_synchronous=True, allow_fallback_pdf=True,
                                       bypass_download=False, **kwargs):
        ...
        return composer.action_send_and_print(force_synchronous=force_synchronous,
                                              allow_fallback_pdf=allow_fallback_pdf, bypass_download=bypass_download)
    ...
    def _get_invoice_legal_documents(self):
        ...
        if self.invoice_pdf_report_id:
            ...
            content, _ = self.env['ir.actions.report']._render('account.account_invoices', self.ids,
                                                               data={'proforma': True})
    ...
    def get_backend_url(self):
        ...
        try:
            if self.move_type in ('out_invoice', 'out_refund'):
                ...
                action = self.env.ref('account.action_move_all_account_entries')
            return f"{base_url}/web#id={self.id}&model=account.move&view_type=form&action={action.id}"
        except Exception:
    ...
    def button_wait(self):
        ...
        group = self.env.ref('account.group_account_full_manager', raise_if_not_found=False)
        template = self.env.ref('account.email_template_invoice_validation', raise_if_not_found=False)

        if not group or not template:
            raise UserError(_("Groupe ou modèle d'e-mail introuvable."))
